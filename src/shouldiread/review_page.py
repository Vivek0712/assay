...
.pub {{ color:var(--muted); font-size:12.5px; margin:3px 0 0; }}
.pub a, .title a {{ color:inherit; }}
...
function render(d) {{
  ...
  return `<div class="result ${{esc(d.verdict)}}">
    ...
    <p class="title">${{d.url
        ...
        : esc(d.title || '')}}</p>
    <p class="pub">${{d.published_at ? `published ${{new Date(d.published_at)
        .toLocaleDateString(undefined, {{day:'numeric', month:'short', year:'numeric'}})}}` : ''}}${{
      d.url ? ` &middot; <a href="${{esc(d.url)}}" target="_blank" rel="noopener">open on Builder Center &rarr;</a>` : ''}}</p>
